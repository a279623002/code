...
输入文本 → 嵌入 + 位置编码 → 
N层 Encoder 编码原文特征
↓（Encoder 输出 K/V 传入 Decoder 交叉注意力）
右移目标序列 → 嵌入 + 位置编码 → 
N层 Decoder（掩码自注意力→交叉注意力→FFN）→ Linear（线性层） → Softmax（归一化层） → 输出每个词的生成概率
...
* <strong>参考答案：</strong>
    ...
    4. N 层堆叠 Encoder 单层内部顺序（每层 2 个子层，均带残差 Add & Norm），每层固定流程：
        多头自注意力 Multi-Head Attention：无掩码，所有 Token 互相可见，双向建模上下文；
        Add & Norm：残差连接（输入直接加到注意力输出）+ 层归一化；
        前馈网络 Feed Forward：逐 Token 独立两层全连接，升维再降维；
