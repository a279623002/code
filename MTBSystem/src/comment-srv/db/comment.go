...
func SelectHotComment(movieId int64) ([]*entity.Comment, error) {
	...
	err := db.Select(&comments, "SELECT * FROM `comment` WHERE film_id = ?", movieId)
...
func InsertHotComment(comment *entity.Comment) error {
	...
	_, err := db.Exec("INSERT INTO `comment`(`film_id`,`title`,`content`,`head_img`,`nick_name`,`create_at`, `user_id`) VALUES (?,?,?,?,?,?,?)", comment.FilmId, comment.Title, comment.Content, comment.HeadImg, comment.NickName, today, comment.UserId)
	return err
}
